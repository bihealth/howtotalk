...
title-slide-attributes:
  data-background-image: images/IMGP6272b-widok.jpg
...
The newest version of this presentation is available at
[bihealth.github.io/howtotalk](https://bihealth.github.io/howtotalk).

You can find the sources of the presentation (Qmd file) on 
[github.com/bihealth/howtotalk](https://github.com/bihealth/howtotalk)


I have elaborated parts of this talk into a little brochure, to be found at
[bihealth.github.io/howtotalk-book](https://bihealth.github.io/howtotalk).
...
power <- map_dfr(n_samp, ~ {
  ...
  mutate(n_samples_tot = 2 * n_samples)
```

```{r echo=FALSE,cache=TRUE}
#| label: power_int

# power calculation for interaction term
# we could use pwr package here, but simulation is more explicit
library(broom)
...
power_int <- map_dfr(n_samp, ~ {
  ...
  mutate(n_samples_tot = 4 * n_samples)
```
 ...
 * Treatment + control
 * Does treatment have a different effect on the KO
...
::: {.notes}
Note that this is not questioning the finding that you do not need to
increase sample size when you switch from 1-sex to 2-sex designs - as long
as you are not interested in the interaction term!
:::
...
flowchart LR
    ...
    AAAA("Biological question") --> B(Data import)
    B --> C[Data cleanup]
    C --> CC[QC]
    CC --> E[Analysis]
    E --> D
    E --> F(Figures)
    E --> G(Manuscript fragments)
    E --> H(Tables, Excel files)
    F --> I[You]
    G --> I
    H --> I
    I --> J[Fine tuning]
...
I have started writing a little brochure based on this presentation, you
can find the current progress (about 10% done) at
